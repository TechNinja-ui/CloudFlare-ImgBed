...
export async function onRequest(context) {
    ...
    if (request.method === 'GET') {
        ...
        return new Response(JSON.stringify(maskUploadConfig(settings)), {
            headers: {
...
function maskUploadConfig(settings) {
    return {
        ...settings,
        telegram: {
            ...settings.telegram,
            channels: (settings.telegram?.channels || []).map(channel => ({
                ...channel,
                botToken: maskSecret(channel.botToken),
            })),
        },
        s3: {
            ...settings.s3,
            channels: (settings.s3?.channels || []).map(channel => ({
                ...channel,
                accessKeyId: maskSecret(channel.accessKeyId),
                secretAccessKey: maskSecret(channel.secretAccessKey),
            })),
        },
        discord: {
            ...settings.discord,
            channels: (settings.discord?.channels || []).map(channel => ({
                ...channel,
                botToken: maskSecret(channel.botToken),
            })),
        },
        huggingface: {
            ...
            channels: (settings.huggingface?.channels || []).map(channel => ({
                ...channel,
                token: maskSecret(channel.token),
            })),
        },
        webdav: {
            ...settings.webdav,
            channels: (settings.webdav?.channels || []).map(channel => ({
                ...channel,
                username: maskSecret(channel.username),
                password: maskSecret(channel.password),
                headers: maskHeaders(channel.headers),
            })),
        },
    };
}
...
function maskHeaders(headers = {}) {
    ...
    for (const [key, value] of Object.entries(headers || {})) {
        result[key] = /authorization|token|secret|key|cookie/i.test(key) ? maskSecret(value) : value;
    }
    return result;
}
