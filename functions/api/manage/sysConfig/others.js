...
export async function onRequest(context) {
    ...
    if (request.method === 'GET') {
        ...
        return new Response(JSON.stringify(maskOthersConfig(settings)), {
            headers: {
...
function maskOthersConfig(settings) {
    return {
        ...settings,
        cloudflareApiToken: {
            ...settings.cloudflareApiToken,
            CF_EMAIL: maskSecret(settings.cloudflareApiToken?.CF_EMAIL),
            CF_API_KEY: maskSecret(settings.cloudflareApiToken?.CF_API_KEY),
        },
        webDAV: {
            ...settings.webDAV,
            username: maskSecret(settings.webDAV?.username),
            password: maskSecret(settings.webDAV?.password),
            internalToken: maskSecret(settings.webDAV?.internalToken),
        },
    };
}
