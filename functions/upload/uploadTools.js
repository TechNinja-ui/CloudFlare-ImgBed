...
export async function buildUniqueFileId(context, fileName, fileType = 'application/octet-stream') {
    ...
    // 所有上传统一强制重命名，避免在 URL/对象键中暴露原始文件名。
    // 保留扩展名用于内容类型识别和用户体验。
    if (nameType === 'short') {
        while (true) {
    ...
    baseId = normalizedFolder
        ? `${normalizedFolder}/${uniqueIndex}_${randomSuffix}.${fileExt}`
        : `${uniqueIndex}_${randomSuffix}.${fileExt}`;
    ...
    while (true) {
        ...
        const baseName = `${uniqueIndex}_${randomSuffix}`;
